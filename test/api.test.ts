...
describe('HAICO API', () => {
  ...
  describe('Dashboard Summary', () => {
    ...
    it('GET /api/dashboard/activity-stream supports project_id filter', async () => {
      ...
      assert.ok(Array.isArray(body));
      for (const evt of body) {
        assert.equal(evt.project_id, projectId, 'filtered events should belong to the requested project');
      }
    });
    ...
    it('GET /api/dashboard/activity-stream requires auth', async () => {
      const res = await inject(app, { url: '/api/dashboard/activity-stream' });
      assert.equal(res.statusCode, 401);
    });

    it('activity-stream includes approval events with agent_name (#627)', async () => {
      // Create an approval request via the API
      const { status: createStatus, body: approval } = await api(app, `/api/projects/${projectId}/approvals`, {
        method: 'POST',
        body: {
          agent_id: controllerId,
      ...
      const approvalEvt = body.find((e: any) => e.event_type === 'approval_created' && e.object_id === approval.id);
      assert.ok(approvalEvt, 'activity stream should include the approval event');
      assert.ok(approvalEvt.agent_name, 'approval event should have agent_name from joined agents table');
    });
    ...
    it('GET /api/dashboard/agents returns agent list (with auth)', async () => {
      ...
      for (const agent of body) {
        ...
        assert.ok(typeof agent.status === 'string', 'each agent should have status');
    ...
    it('GET /api/dashboard/agents supports status filter', async () => {
      ...
      assert.ok(Array.isArray(body));
      for (const agent of body) {
        assert.equal(agent.status, 'idle', 'filtered agents should have the requested status');
      }
    });

    it('GET /api/dashboard/agents requires auth', async () => {
      const res = await inject(app, { url: '/api/dashboard/agents' });
      assert.equal(res.statusCode, 401);
    });
    ...
    it('GET /api/dashboard/today-cost returns cost data (with auth)', async () => {
      ...
      assert.ok(typeof body.by_project === 'object', 'should have by_project object');
    });

    it('GET /api/dashboard/today-cost requires auth', async () => {
      const res = await inject(app, { url: '/api/dashboard/today-cost' });
      assert.equal(res.statusCode, 401);
    });
