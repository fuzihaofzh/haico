# Htmx SSR 片段实现指南
<!-- depends-on: AGENTS.md, htmx-ssr-fragments.md#三层路径结构, htmx-ssr-fragments.md#Views层规则, frontend.md#HTML Helper, conventions.md#API约定 -->
<!-- L2: 路由文件结构, h/html/renderToString helper, Shell SSR 模式, 片段端点模式, Toast与Confirm, CSS规则, 测试模式, 迁移检查清单, 常见错误速查 -->

本文件是 `htmx-ssr-fragments.md` 的 L2 实现段，承载 admin 三页面 htmx 迁移的具体模式、代码示例与检查清单。L1 契约（何时用、三层路径结构、Views 层规则）见 `htmx-ssr-fragments.md`。
